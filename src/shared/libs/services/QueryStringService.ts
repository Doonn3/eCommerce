...
class QueryStringService {
  ...
  public addQueryParam(name: string, value: string): void {
    ...
    const newQueryParamValuesString: string = urlParams.has(name)
      ? `${urlParams.get(name)} ${value}`
      : value;
  ...
  public deleteQueryParam(name: string, value: string): void {
    ...
    if (queryParamValues.length === 1) {
      ...
      const newQueryParamValues: string[] = queryParamValues.filter(
        (queryParamValue) => queryParamValue !== value
      );
